...
def balanced(m):
    ...
    if total_weight(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m)):
        if is_planet(end(left(m))) and is_planet(end(right(m))):
            ...
            if not is_planet(end(left(m))) and not is_planet(end(right(m))):
                return balanced(end(left(m))) and balanced(end(right(m)))
            elif not is_planet(end(left(m))):
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, user_password):
        nonlocal balance, password, incorrect_pw_list
        if user_password != password:
            if len(incorrect_pw_list) < 3:
                incorrect_pw_list.append(user_password)
                return 'Incorrect password'
        if len(incorrect_pw_list) == 3:
            return f"Your account is locked. Attempts: {incorrect_pw_list}"
        if amount > balance:
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    original_account = withdraw(0, old_pass)
    if type(original_account) == str:
        return original_account
    joint_pw = [old_pass, new_pass]

    def joint_withdraw(amount, user_password):
        if user_password in joint_pw:
...
def multiple_references_explanation():
    return """in par1, r1 may refer to different values so as r2; while in par2."""
